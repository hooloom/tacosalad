...
@Directive({
  ...
  selector: '[columnmiddledirective]'
})
export class columnmiddleDirective implements OnInit, OnDestroy, AfterViewInit {
  @Input() columnmiddledirective: any;
  orgdata: any;
  components: any;
  route0$: any;
  ...
  ngAfterViewInit() {
    ...
    this.loadComponent(this.columnmiddledirective);
    ...
    if (components) {
     console.log(components)
      ...
      const filtered = from(sortedcomponents).pipe(
        filter(component => {
          if (
            component.published === true && componentmanifest.pagespublished === route0 || component.da === "contentoutlet" ) {
              return component;
          } 
        })
      );
