...
export class contentDirective implements OnInit, OnDestroy, AfterViewInit {
  @Input() contentdirective: any;
  orgdata: any;
  components: any;
  route0$: any;
  ...
  ngAfterViewInit() {
    ...
    this.loadComponent(this.contentdirective);
  ...
  loadComponent(data: any) {
    ...
    if (components) {
      ...
      const filtered = from(sortedcomponents).pipe(
        filter(component => {
          if (
            component.da=="content" ||  component.published === true && componentmanifest.pagespublished === route0 ) {
              return component;
          } 
        })
